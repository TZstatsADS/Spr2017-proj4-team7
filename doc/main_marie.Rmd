...
- There are 14 .txt files in the data folder. Each file is a collection of ambiguous names and associtated citations. e.g. AGupta.txt is the citation files of 26 â€œA. Guptaâ€s. The 14 canonical names are top ranked ambiguous names, such as â€œJ. Leeâ€?, â€œJ Smithâ€?, â€œS. Leeâ€and â€œY. Chenâ€? from the DBLP bibliography.
- The datasets are pre-processed as follows. All the author names in the citations were simpli- fied to first name initial and last name. For example, â€œYong-Jik Kimâ€? was simplified to â€œY. Kimâ€?. A reason for such simplification is that the first name initial and last name format is popular in citation records. Publication dates are eliminate from citations.
...
Letâ€™s first create a vocabulary-based DTM. Here we collect unique terms from all records and mark each of them with a unique ID using the  `create_vocabulary()` function. We use an iterator to create the vocabulary.
```{r}
it <- itoken(df$paper, 
...
vocab <- create_vocabulary(it, stopwords = c("a", "an", "the", "in", "on",
                                             "at", "of", "above", "under"))
```
Here, we remove pre-defined stopwords, the words like â€œaâ€?, â€œtheâ€?, â€œinâ€?, â€œIâ€?, â€œyouâ€?, â€œonâ€?, etc, which do not provide much useful information. 
