...
```{r}
svm.fit <- SVM(author.id~., data=dtm.train, 
...
Given the results of this analysis, we will be using the ** scheme with a ** kernel. 
...
for (attribute in c("paper", "coauthors", "journal", "hybrid")){
  for (name_set in 1:length(dataset)){
    results[name_set, attribute] <- run.model(name_set, attribute, dataset)
  }
}
```
